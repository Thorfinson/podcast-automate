...
- Umgebung: `subscription_environment()` aus `codex.py` erweitern (zusätzlich `ANTHROPIC_API_KEY`, `ANTHROPIC_AUTH_TOKEN` entfernen) und `DISABLE_TELEMETRY=1`, `DISABLE_ERROR_REPORTING=1`, `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1` und `CLAUDE_CODE_MAX_OUTPUT_TOKENS=64000` setzen (die Obergrenze der CLI für diese Modellkennung; ohne die Einstellung 32 000). `CLAUDE_CONFIG_DIR` bleibt unverändert, weil dort die Abo-Zugangsdaten liegen.
...
- Fehlerklassen (`classify_claude_failure`): Ergebnistext oder `subtype` mit „You've hit your“, „limit“, „rate limit“, „429“ → `AppError(code="claude_quota_exhausted", status="waiting_for_quota")` mit geparstem Reset-Zeitpunkt; „not logged in“, „authentication“, „401“ → `authentication_required`; `--max-budget-usd` erreicht → `claude_budget_cap`; abgeschnittene Antwort (`assistant.error == "max_output_tokens"` oder „output token maximum“ im Ergebnistext, Ergebnis `success` mit `is_error` und Exit-Code 1) → `claude_output_limit`, `blocked`; alles andere `claude_failed`. Rohtexte werden wie bei Codex nicht gespeichert, nur `failure.json` mit Code und Version.
...
- **Ausgabelimit je Antwort.** Die CLI kennt `claude-opus-5` nicht in ihrer Modelltabelle und begrenzt die Ausgabe ohne Einstellung auf 32 000 Token; ein Dossier über acht geprüfte Antworten hat das am 20.09.2026 überschritten. Der Adapter setzt darum `CLAUDE_CODE_MAX_OUTPUT_TOKENS=64000`, eine abgeschnittene Antwort endet als `claude_output_limit` und wird nicht übernommen, und die Synthese begrenzt das Material je komponierendem Aufruf (`question_synthesis.ANSWER_BUDGET_CHARS`).
...
**Nachtrag 26.09.2026:** Claude Code auf 2.1.283 und Codex CLI auf 0.157.1 (npm) aktualisiert. Der Claude-Standard ist jetzt `claude-opus-5-5` mit `xhigh` (2.1.92 lehnt beides ab; die Mindestversion des Adapters ist 2.1.280). Die Auswahlregel `auto` fragt neue Läufe zuerst nach Claude (`prefer: claude_code`) und wechselt erst bei erschöpftem Claude-Kontingent zu Codex; gespeicherte Läufe behalten ihre Reihenfolge. Der Stream-Umschlag mit `structured_output` ist mit 2.1.283 unverändert (Textprobe mit Opus 5.5 auf `xhigh`). Die CLI meldet für Opus 5.5 ein Kontextfenster von 1 000 000 und 128 000 Ausgabetokens; Adapter-Ausgabedeckel (64 000) und Promptgrenze (300 000 Zeichen) sind unverändert.
