...
Diese Seite bewahrt die ausführlichen Funktionsnotizen der bisherigen README in ihrer ursprünglichen Reihenfolge. Die README selbst ist seit dem 19.09.2026 ein kurzer Überblick; die verbindlichen Anleitungen stehen unter `docs/`. Aussagen über Testergebnisse und Meilensteine gelten für das jeweils genannte Datum.
...
**Windows:** `Podcast-Studio.cmd` doppelklicken. **macOS:** nach der Einrichtung `Podcast-Studio.command` doppelklicken. **Linux:** `sh Podcast-Studio.sh`. Die Startseite zeigt alle Projekte, ihre Arbeitsstände und sämtliche fertigen Folgen mit Player und MP3-Download. Dort legst du Projekte an, öffnest sie oder verschiebst sie in den lokalen Papierkorb; Wiederherstellen ist möglich. Innerhalb eines Projekts bleiben sechs Schritte: **Auftrag & Stimmen → Recherche → Inhaltsverzeichnis → Ausarbeitung → Skripte lesen → Vertonung**. Das Serverfenster bleibt geöffnet.
...
Unter **Auftrag & Stimmen** sammelt ein einzelner redaktioneller Partner deine Wünsche im Chat. Er fragt nach Thema, Tiefe, Sprache, Stimmen, Textmodell und Denkaufwand (`low`, `medium`, `high`, `xhigh`). Konfigurationsformulare entfallen; du prüfst die Zusammenfassung und klickst **Diese Auswahl übernehmen**. Ein separater geschützter Key-Eingang hält Zugangsdaten aus dem Gespräch heraus. Neue Codex-Aufträge haben **GPT-6 Astra mit xhigh** als Vorauswahl; eigene Modell-IDs sind ebenfalls möglich. Modell und Stufe werden ausdrücklich übergeben, mit dem Auftrag gespeichert und beim Fortsetzen beibehalten. Die Auswahl gilt für Lehrkonzept, Skript, Polishing und Prüfungen; bei Codex auch für Recherche und Nachrecherche. OpenRouter-Text unterstützt eine optionale Reasoning-Stufe, sofern das Modell sie akzeptiert. Ein schon laufender Studio-Server benötigt nach diesem Update einen Neustart, sobald sein Auftrag fertig ist.

Die Navigation zeigt gespeicherte Ergebnisse, laufende Arbeit und anstehende Freigaben. Unter **Ausarbeitung** stehen Lehrkonzept, Skriptentwurf, Dialog-Polishing und Qualitätsprüfung mit aktuellem Status und Fortschritt je Folge. Geprüfte Lehrkonzepte lassen sich dort aufklappen und lesen. Auf anderen Seiten bleibt ein kompakter Status mit einem Link zum laufenden Schritt. Die Anzeige aktualisiert sich automatisch; ein einzelner Modellaufruf kann mehrere Minuten dauern. Projekt und gewählte Seite bleiben beim Neuladen über die Seitenadresse erhalten. Beim Öffnen ohne Seitenvorgabe erscheint die Projektübersicht; „Projekt öffnen“ führt zum passenden Arbeitsschritt. Eine manuell gewählte Leseseite bleibt auch dann geöffnet, wenn die Hintergrundarbeit fertig wird.
...
Unter **Skripte lesen** ist jede Folge schon ab dem vollständig gespeicherten Entwurf lesbar, während die übrigen entstehen. Die Ansicht unterscheidet Entwurf, abgeschlossene Dialogüberarbeitung und bestandene Prüfungen. Neue Folgen erscheinen automatisch; der gerade gelesene Text bleibt stehen, bis du einen neueren Stand ausdrücklich lädst. Diese Vorschauen erteilen keine Audio-Freigabe und ändern keine Veröffentlichungs- oder Prüfdateien.
...
Das Studio ist für Windows, macOS und Linux eingerichtet. Der bisherige Hardware-Pilot lief auf Windows 11 mit einer AMD Radeon RX 9070 XT; echte Qwen-Hörtests auf Mac und Linux stehen noch aus. Die belegte Recherche verwendet Codex CLI mit dem vorhandenen ChatGPT-Abo. Für Redaktion und Skripte ist zusätzlich OpenRouter wählbar; die Audioauswahl zwischen lokalem Qwen und Gemini über OpenRouter ist davon unabhängig. Installation, Gerätewahl und Grenzen beim Rechnerwechsel stehen in der [macOS-/Linux-Anleitung](docs/macos-linux.md). Die technischen Grundlagen und Quellen stehen im [Implementierungsplan](docs/personal-learning-podcast-system-plan.md).
...
- [docs/macos-linux.md](docs/macos-linux.md): Installation, Starter, optionale Qwen-Geräte und Rechnerwechsel.
