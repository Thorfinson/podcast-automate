...
| --- | --- | --- | --- |
| Python | `python -m unittest discover -s tests` | about 410 tests, about 100 s | `ffmpeg` and `ffprobe` on PATH |
| Browser logic | `node --test tests/studio_ui.test.cjs` | 67 tests, under 1 s | Node 22 |
...
- `cli`, `doctor` → `test_cli`; resume through the CLI is also exercised by `test_episode_audio`, `test_openrouter`, `test_research`, `test_scripting`
- `codex`, `codex_stream`, `call_activity`, `model_trace` → `test_codex`, `test_codex_stream`, `test_setup_schema`, `test_model_trace`, `test_status_summary`, `test_studio_progress`
...
- `research` and every `research_*` module → `test_research`, `test_research_invariants`, `test_research_migration`, `test_research_quality`, `test_research_refinement`, `test_research_status`, `test_evidence_contracts`, `test_question_research`, `test_attachments`, `test_parallel`, `test_run_budget`, `test_prompts`
- `run_budget`, `script_budget`, `script_checkpoints` → `test_run_budget`, `test_script_budget`, `test_question_research`, `test_studio_progress`, `test_teaching_research`
- `scripting`, `script_pipeline`, `script_checks`, `script_artifacts`, `script_models`, `script_evidence` → `test_scripting`, `test_teaching`, `test_teaching_research`, `test_polishing`, `test_parallel`, `test_planning`, `test_run_budget`, `test_series_review`, `test_episode_framing`, `test_openrouter`, `test_speech`, `test_studio`, `test_studio_scripts`, `test_text_selection`, `test_evidence_contracts`
- `series_review` → `test_series_review`, `test_scripting`
- `sources`, `downloads` → `test_downloads`, `test_attachments`, `test_evidence_contracts`, `test_question_research`, `test_research`, `test_research_migration`, `test_teaching_research`
...
- `status_summary`, `research_status` → `test_status_summary`, `test_research_status`
- `studio`, `studio_worker`, `studio_progress`, `studio_scripts` → `test_studio`, `test_studio_progress`, `test_studio_scripts`, `test_studio_trash`, `test_attachments`, `test_parallel`, `test_platforms`, `test_setup_schema`, `test_speech`, `test_text_selection`, `test_question_research`, `test_run_budget`
