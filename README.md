...
**`Podcast-Studio.cmd` doppelklicken.** Das lokale [Podcast Studio](http://127.0.0.1:8765) führt durch **Auftrag & Stimmen → Recherche → Inhaltsverzeichnis → Ausarbeitung → Skripte lesen → Audio & Export**. Das Serverfenster bleibt geöffnet. Alternativ startet `.\.venv\Scripts\pla.exe studio` dieselbe Oberfläche.
